...
from orbit.capture_service_registry import (
    register_capture_service,
    unregister_capture_service,
)
...
class CaptureDispatcherTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_enqueue_reuses_registered_app_service_for_audio_stream_ownership(self):
        service = MagicMock()
        service.start_meeting_capture_session = AsyncMock(return_value={"status": "started"})
        register_capture_service(service)

        with patch("orbit.capture_dispatcher.OrbitWhatsAppService") as service_cls:
            await enqueue_meeting_capture(
                VALID_MEETING_ID,
                gmeet_url="https://meet.google.com/abc-defg-hij",
                capture_session_id=VALID_CAPTURE_SESSION_ID,
